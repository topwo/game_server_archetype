...
#set( $symbol_dollar = '$' )
#set( $symbol_escape = '\' )
package com.kidbear.${artifactId}.util.hibernate;
...
import com.kidbear.${artifactId}.util.cache.MC;
import com.kidbear.${artifactId}.util.cache.MCSupport;
...
public class HibernateUtil {
    ...
    public static <T> T find(Class<T> t, long id) {
        ...
        if (ret == null) {
            if (showMCHitLog)
                log.info("MC未命中{}${symbol_pound}{}", t.getSimpleName(), id);
            ret = find(t, "where " + keyField + "=" + id, false);
            if (ret != null) {
                if (showMCHitLog)
                    log.info("DB命中{}${symbol_pound}{}", t.getSimpleName(), id);
                MC.add(ret, String.valueOf(id));
            } else {
                if (showMCHitLog)
                    log.info("DB未命中{}${symbol_pound}{}", t.getSimpleName(), id);
            }
        } else {
            if (showMCHitLog)
                log.info("MC命中{}${symbol_pound}{}", t.getSimpleName(), id);
        }
        return ret;
    }
    ...
    public static <T> T findByName(Class<? extends MCSupport> t, String name,
        ...
        if (ret == null) {
            ...
            log.info("found id from DB {}${symbol_pound}{}", targetClz.getSimpleName(), mcId);
    ...
    public static <T> List<T> list(Class<T> t, long id, String where) {
        ...
        if (ret == null) {
            if (showMCHitLog)
                log.info("MC未命中{}${symbol_pound}{}", t.getSimpleName(), where);
            ret = list(t, where, false);
            if (ret != null) {
                if (showMCHitLog)
                    log.info("DB命中{}${symbol_pound}{}", t.getSimpleName(), where);
                MC.addList(ret, t.getSimpleName(), String.valueOf(id), where);
            } else {
                if (showMCHitLog)
                    log.info("DB未命中{}${symbol_pound}{}", t.getSimpleName(), where);
            }
        } else {
            if (showMCHitLog)
                log.info("MC命中{}${symbol_pound}{}", t.getSimpleName(), where);
        }
        return ret;
    }
    ...
    public static boolean synMC4Insert(String tName, Object o, String id) {
        ...
        if (keys != null && keys.length() > 0) {// 遍历所有的where缓存
            ...
            for (String where : wheres) {
                where = where.split("${symbol_pound}")[3];
