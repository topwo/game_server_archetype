package com.kidbear.pay.client.pay;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.kidbear.pay.util.Constants;
...
public class PayController implements IPay {
    ...
    public long generateOrder(Pay pay) {
        ...
        pay.setIsFinished(PayCode.NOT_VALIDATE);
        MC.add(pay, "" + pay.getIdentifier());
        HibernateUtil.insert(pay, pay.getUserid());
        // 发回订单号billno
        return billno;
    }
    ...
    public boolean sendGoods(long billno, boolean success) {
        if (success) {
            Pay pay = HibernateUtil.find(Pay.class, billno);
            pay.setIsFinished(PayCode.FINISH);// 支付成功，发货成功
            HibernateUtil.save(pay, billno);
    ...
    public int queryPaySum(long userid) {
        List<Pay> payList = HibernateUtil.list(Pay.class, "where userid='"
                + userid + "'");
        int sum = 0;
        for (Pay pay : payList) {
            if (pay.getIsFinished() == PayCode.FINISH) {
                sum += pay.getAmount();
            }
        }
        return sum;
    ...
    public String queryOrder(HttpServletRequest request) {
        ...
        Pay pay = HibernateUtil.find(Pay.class,
                "where billno=" + request.getParameter("billno") + "");
        String ret = pay == null ? "" : JSON.toJSONString(pay);
        return ret;
    }
    ...
    public String generateOrder(HttpServletRequest request) {
        ...
        long billno = TableIDCreator.getTableID(Pay.class, 1);
        Pay pay = new Pay();
        pay.setUserid(Integer.parseInt(request.getParameter("userid")));
        pay.setZone((int) (pay.getUserid() % 1000));
        pay.setGoodType(Integer.parseInt(request.getParameter("goodType")));
        pay.setGamename(request.getParameter("gamename"));
        pay.setChannel(request.getParameter("channel"));
        pay.setBillno(billno);
...
//		}
//		if (!flag) {
//			pay.setIsFinished(PayCode.VALIDATE_ERR);// 验证失败
//			HibernateUtil.save(pay, pay.getUserid());
//			ret.put("ret", 6);
//			ret.put("msg", "君主" + pay.getUserid() + "支付验证失败");
//			return write(ret);
//		}
//		pay.setAccount(uid);
//		pay.setIsFinished(PayCode.NOT_SEND);// 支付成功
//		long serverId = pay.getUserid() % 1000;
...
//						new String[] { pay.getGoodType() + "",
//								pay.getUserid() + "" }, Boolean.class);
...
//		}
//		if (serverFlag) {
//			pay.setIsFinished(PayCode.FINISH);
//			HibernateUtil.save(pay, pay.getUserid());
//			ret.put("ret", 0);
//			ret.put("msg", "发货成功");
//			return write(ret);
//		} else {
//			pay.setIsFinished(PayCode.SEND_ERR);// 发货失败
//			HibernateUtil.save(pay, pay.getUserid());
//			ret.put("ret", 8);
//			ret.put("msg", "发货失败");
			return write(ret);
...
//					return write(ret);
//				}
//				pay.setAccount(signParams.get("uid"));
//				pay.setIsFinished(PayCode.NOT_SEND);// 支付成功
//				long serverId = pay.getUserid() % 1000;
//				String serverRet = HttpClient.get("http://"
//						+ GameInit.cfg.get("loginServer") + ":"
...
//				pay.setIsFinished(PayCode.VALIDATE_ERR);// 验签失败
//				HibernateUtil.save(pay, pay.getUserid());
//				ret.put("errcode", resultCode);
//				ret.put("errMsg", "君主" + pay.getUserid() + "支付验签失败");
//				return write(ret);
//			}
//		} catch (Exception e) {
...
//		orderReq.append("</request>");
//		String urlStr = "http://api.gfan.com/sdk/pay/queryAppPayLog";
