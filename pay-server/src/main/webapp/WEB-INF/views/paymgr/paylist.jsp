                        ...
                        <c:otherwise>
                            <c:forEach items="${payList }" var="pay">
                                <tr>
                                    ...
                                    <td>${pay.gamename }</td>
                                    <td><c:choose>
                                        <c:when test="${pay.goodType==1 }">首冲</c:when>
                                        <c:when test="${pay.goodType==2 }">普通会员</c:when>
                                        <c:when test="${pay.goodType==3 }">超级会员</c:when>
                                        <c:when test="${pay.goodType==4 }">豪华签到</c:when>
                                        <c:when test="${pay.goodType==5 }">购买元宝14888</c:when>
                                        <c:when test="${pay.goodType==6 }">购买元宝6888</c:when>
                                        <c:when test="${pay.goodType==7 }">购买元宝3888</c:when>
                                        <c:when test="${pay.goodType==8 }">购买元宝1888</c:when>
                                        <c:when test="${pay.goodType==9 }">购买元宝1088</c:when>
                                        <c:when test="${pay.goodType==10 }">购买元宝388</c:when>
                                        <c:when test="${pay.goodType==11 }">普通玩家每日折扣</c:when>
                                        <c:when test="${pay.goodType==12 }">普通会员每日折扣</c:when>
                                        <c:when test="${pay.goodType==13 }">超级会员每日折扣</c:when>
                                        <c:when test="${pay.goodType==14 }">神将礼包</c:when>
                                    </c:choose></td>
                                    <td>${pay.payDate }</td>
                                    <td><c:choose>
                                        <c:when test="${pay.isFinished==0 }">待验证</c:when>
                                        <c:when test="${pay.isFinished==1 }">验证失败</c:when>
                                        <c:when test="${pay.isFinished==2 }">未发货</c:when>
                                        <c:when test="${pay.isFinished==3 }">发货失败</c:when>
                                        <c:when test="${pay.isFinished==4 }">订单完成</c:when>
                                    </c:choose></td>
                                </tr>
                            </c:forEach>
