...
<body>
	...
	<div id="summary">
		...
		<ul>
			...
			<c:forEach items="${channelValMap }" var="basic">
				<li><span class="channel">${basic.key }</span>: <span><fmt:formatNumber
							type="number" value="${basic.value }" maxFractionDigits="0" /></span></li>
			</c:forEach>
		</ul>
	</div>
	<!-- ECharts单文件引入 -->
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						for ( var tmp in ret.basicChartDatas) {
							ret.basicChartDatas[tmp].name = replaceChannelName(ret.basicChartDatas[tmp].name);
						}
						// 路径配置
						require.config({
							paths : {
