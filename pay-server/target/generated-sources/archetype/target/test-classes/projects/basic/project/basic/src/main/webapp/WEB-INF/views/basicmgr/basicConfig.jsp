...
.basicConfigLayer ul li {
	margin: 20px;
	height: 40px;
	line-height: 40px;
}

.basicConfigLayer .btn {
	width: 100%;
	height: 50px;
	line-height: 50px;
...
<script type="text/javascript">
	function update() {
		...
		$.ajax({
			url : "../basicmgr/basicConfigHandle",
			type : "post",
			data : datas,
			success : function(data) {
				if(data='success'){
					alert("修改支付配置成功");
					$("#right").load("../basic/basicConfig");
				}
			}
		});
	}
