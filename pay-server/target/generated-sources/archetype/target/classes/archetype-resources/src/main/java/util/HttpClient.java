...
public class HttpClient {
	 ...
	 * @throws IOException
	 */
	private static HttpURLConnection initHttp(String url, String method, Map<String, String> headers, String contentType)
			throws IOException {
	...
	private static HttpsURLConnection initHttps(String url, String method, Map<String, String> headers,
			String contentType) throws IOException, NoSuchAlgorithmException, NoSuchProviderException,
	...
	public static String post(String url, String params, String contentType) {
		try {
			...
			OutputStreamWriter out = new OutputStreamWriter(http.getOutputStream(),DEFAULT_CHARSET);
	...
	byte[] end_data = ("${symbol_escape}r${symbol_escape}n--" + BOUNDARY + "--${symbol_escape}r${symbol_escape}n").getBytes();// 定义最后数据分隔线
	StringBuilder sb = new StringBuilder();
	sb.append("--");
	sb.append(BOUNDARY);
	sb.append("${symbol_escape}r${symbol_escape}n");
	sb.append("Content-Disposition: form-data;name=${symbol_escape}"media${symbol_escape}";filename=${symbol_escape}"" + file.getName() + "${symbol_escape}"${symbol_escape}r${symbol_escape}n");
	sb.append("Content-Type:application/octet-stream${symbol_escape}r${symbol_escape}n${symbol_escape}r${symbol_escape}n");
	...
	out.write("${symbol_escape}r${symbol_escape}n".getBytes()); // 多个文件时，二个文件之间加入这个
	fs.close();
	out.write(end_data);
	out.flush();
	out.close();
	...
	public static String upload(String url, File file,String imgUrl) throws IOException, NoSuchAlgorithmException,
			NoSuchProviderException, KeyManagementException {
		String BOUNDARY = "----WebKitFormBoundaryiDGnV9zdZA1eM1yL"; // 定义数据分隔线
		StringBuffer bufferRes = null;
		URL urlGet = new URL(url);
...
/*	public static Attachment download(String url) throws IOException {
		...
		HttpURLConnection conn = initHttp(url, _GET, null,CONTENT_YPTE);
		if (conn.getContentType().equalsIgnoreCase("text/plain")) {
			...
			String fullName = ds.substring(ds.indexOf("filename=${symbol_escape}"") + 10, ds.length() - 1);
