...
<script type="text/javascript">
	function saveServer() {
		...
		var ip = ${symbol_dollar}('${symbol_pound}ip').val();
		var port = ${symbol_dollar}('${symbol_pound}port').val();
		var tomcatPort = ${symbol_dollar}('${symbol_pound}tomcatPort').val();
		var state = ${symbol_dollar}('${symbol_pound}state').val();
		var max = ${symbol_dollar}('${symbol_pound}max').val();
		var remark = ${symbol_dollar}('${symbol_pound}remark').val();
		var whiteUsers = ${symbol_dollar}('${symbol_pound}whiteUsers').val();
		var openChannels = ${symbol_dollar}('${symbol_pound}openChannels').val();
		var whiteMode = ${symbol_dollar}("input[name='whiteMode']:checked").val() == 'on' ? 1
				: 0;
		if (id == "") {
			alert("请填写服务器id");
			return false;
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<input id="id" placeholder="服务器id" type="number"
									value="${symbol_dollar}{server.id }" name="text-input" class="form-control" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<input id="name" placeholder="服务器名称" type="text"
									value="${symbol_dollar}{server.name }" name="text-input" class="form-control" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<input id="ip" placeholder="服务器ip" type="text"
									value="${symbol_dollar}{server.ip }" name="text-input" class="form-control" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<input id="port" placeholder="服务器端口" type="number"
									value="${symbol_dollar}{server.port }" name="text-input" class="form-control" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<input id="tomcatPort" placeholder="容器端口" type="number"
									value="${symbol_dollar}{server.tomcatPort }" name="text-input"
									class="form-control" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<select id="state" name="select" class="form-control" size="1">
									<option value="0"
										<c:if test="${symbol_dollar}{server.state==0 }">selected="selected"</c:if>>新服</option>
									<option value="1"
										<c:if test="${symbol_dollar}{server.state==1 }">selected="selected"</c:if>>流畅</option>
									<option value="2"
										<c:if test="${symbol_dollar}{server.state==2 }">selected="selected"</c:if>>爆满</option>
									<option value="3"
										<c:if test="${symbol_dollar}{server.state==3 }">selected="selected"</c:if>>维护</option>
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="controls">
								<div class="col-md-1 col-sm-2 col-xs-3">
									<label class="switch switch-success"> <input
										...
										<c:if test="${symbol_dollar}{server.whiteMode==1 }">checked="checked"</c:if>>
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<textarea id="whiteUsers" name="whiteUsers" class="form-control"
									placeholder="请输入白名单账号（或id），格式为'name1,name2,name3,id1,id2,id3'">${symbol_dollar}{server.whiteUsers }</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<textarea id="openChannels" name="openChannels"
									class="form-control" placeholder="请输入开放渠道，格式为'渠道1,渠道2'">${symbol_dollar}{server.openChannels }</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<input id="remark" placeholder="服务器备注" type="text"
									value="${symbol_dollar}{server.remark }" name="text-input"
									class="form-control" />
							</div>
						</div>
						<br>
