...
import ${package}.client.service.AccountService;
import ${package}.core.GameInit;
import ${package}.gm.server.ServerConfig;
import ${package}.util.Constants;
import ${package}.util.hibernate.HibernateUtil;
import ${package}.util.redis.Redis;
...
public class RouteController {
    ...
    public void queryDownUrl(HttpServletRequest request,
        ...
        // TODO 获取版本号val
        String val = "${version}.0${symbol_pound}0";// get from redis or mysql
        String newBase = Redis.getInstance()
                .ztop(Redis.GLOBAL_DB, GameInit.HOT_FIX_BASE + channel, 1)
                .iterator().next().split("${symbol_pound}")[1];
        val = val == null ? "${version}.0${symbol_pound}0" : val;
        version = version == null ? "${version}.0${symbol_pound}0" : version;
        newBase = newBase == null ? "${version}" : newBase;
        int baseInt = Integer.parseInt(base.split("${symbol_escape}${symbol_escape}.")[0]) * 10
                + Integer.parseInt(base.split("${symbol_escape}${symbol_escape}.")[1]);
        int newBaseInt = Integer.parseInt(newBase.split("${symbol_escape}${symbol_escape}.")[0]) * 10
                + Integer.parseInt(newBase.split("${symbol_escape}${symbol_escape}.")[1]);
        int versionInt = Integer.parseInt(version.split("${symbol_escape}${symbol_escape}.")[0]) * 100
                + Integer.parseInt(version.split("${symbol_escape}${symbol_escape}.")[1]) * 10
                + Integer.parseInt(version.split("${symbol_escape}${symbol_escape}.")[2]);
        int newVersionInt = Integer.parseInt(val.split("${symbol_pound}")[0].split("${symbol_escape}${symbol_escape}.")[0])
                * 100 + Integer.parseInt(val.split("${symbol_pound}")[0].split("${symbol_escape}${symbol_escape}.")[1])
                * 10 + Integer.parseInt(val.split("${symbol_pound}")[0].split("${symbol_escape}${symbol_escape}.")[2]);
        if (baseInt > newBaseInt) {
            ...
            if (versionInt > newVersionInt) {
                ...
                // 文件大小
                ret.add(val.split("${symbol_pound}")[1]);
                // 更新版本号
                ret.add(val.split("${symbol_pound}")[0]);
            } else {
                ret.add(0);
            }
        }
    ...
    public void queryVersion(HttpServletRequest request,
        ...
        if (val == null) {
            ...
            writeJSON(val.split("${symbol_pound}")[0], response, request.getRemoteAddr());
        }
    }
