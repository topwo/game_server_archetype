...
import it.pkg.client.model.Account;
import it.pkg.client.service.AccountService;
import it.pkg.core.GameInit;
import it.pkg.gm.server.ServerConfig;
import it.pkg.util.Constants;
import it.pkg.util.hibernate.HibernateUtil;
import it.pkg.util.redis.Redis;
...
public class RouteController {
    ...
    public void queryDownUrl(HttpServletRequest request,
        ...
        // TODO 获取版本号val
        String val = "0.1-SNAPSHOT.0#0";// get from redis or mysql
        String newBase = Redis.getInstance()
                .ztop(Redis.GLOBAL_DB, GameInit.HOT_FIX_BASE + channel, 1)
                .iterator().next().split("#")[1];
        val = val == null ? "0.1-SNAPSHOT.0#0" : val;
        version = version == null ? "0.1-SNAPSHOT.0#0" : version;
        newBase = newBase == null ? "0.1-SNAPSHOT" : newBase;
