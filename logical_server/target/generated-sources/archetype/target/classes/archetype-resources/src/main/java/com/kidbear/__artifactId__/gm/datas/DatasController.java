...
import com.kidbear.${artifactId}.manager.log.RemainInfo;
import com.kidbear.${artifactId}.util.redis.Redis;
...
public class DatasController {
    ...
    public String ipmaplogHandle(HttpServletRequest request) {
        ...
        for (String ip : logMap.keySet()) {
            String country = ip.split("${symbol_pound}").length > 0 ? ip.split("${symbol_pound}")[0] : "";
            String province = ip.split("${symbol_pound}").length > 1 ? ip.split("${symbol_pound}")[1] : "";
            String city = ip.split("${symbol_pound}").length > 2 ? ip.split("${symbol_pound}")[2] : "";
