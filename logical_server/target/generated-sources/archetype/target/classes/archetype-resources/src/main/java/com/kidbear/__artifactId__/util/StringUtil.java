...
public class StringUtil {
	...
	public static String checkString (String str){
		if(str!=null){
			if(str.indexOf(".")!=-1){
				//判断是否为科学计数法
				Pattern pattern = Pattern.compile("^((${symbol_escape}${symbol_escape}d+.?${symbol_escape}${symbol_escape}d+)[Ee]{1}(${symbol_escape}${symbol_escape}d+))${symbol_dollar}");
				Matcher matcher = pattern.matcher(str);
				if(matcher.matches()){
					//if(str.indexOf("E") != -1){
					//String regex = "^((${symbol_escape}${symbol_escape}d+.?${symbol_escape}${symbol_escape}d+)[Ee]{1}(${symbol_escape}${symbol_escape}d+))${symbol_dollar}";
